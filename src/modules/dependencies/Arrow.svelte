...
<script  lang="ts">
    ...
    $: {
        ...
        if (startX + minLen >= endX && startY != endY) {
            result = `L ${startX + minLen} ${startY - 10} 
                        L ${startX + minLen} ${startY + height / 2  - 10}
                        L ${endX - minLen} ${startY + height / 2 - 10}
                        L ${endX - minLen} ${endY - 10} `
        }
        else {
            result = `L ${startX + width / 2} ${startY - 10} 
                        L ${startX + width / 2} ${endY - 10}`
        }

        // -2 so the line doesn't stick out of the arrowhead
        path = `M${startX} ${startY} L ${startX} ${startY - 10}`
